package sync.spctrum.apispring.api.configuration.security;
...
public class AutenticacaoProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(final Authentication authentication) throws AuthenticationException {
        ...
        UserDetails userDetailsGoogle = this.usuarioAutorizacaoService.loadUserByGoogle(username);

        if (this.passwordEncoder.matches(password, userDetails.getPassword())) {
            return new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());

        } else if (Objects.equals(password, userDetailsGoogle.getPassword())) {
            return new UsernamePasswordAuthenticationToken(userDetailsGoogle, null, userDetailsGoogle.getAuthorities());
        } else {
